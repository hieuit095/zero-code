...
## 2024-05-15 - Recursive Component Memoization
**Learning:** When memoizing recursive React components (like a File Tree node) that receive globally active ID props (like `selectedId`), standard shallow equality fails to yield O(1) rendering because the selection state changes on every click.
**Action:** Provide a custom `arePropsEqual` function to `React.memo`. Crucially, branch/folder nodes must *always* return `false` (re-render) to successfully propagate new selection props down the tree. Only memoize leaf/file nodes by comparing if their individual selection state explicitly flipped.
