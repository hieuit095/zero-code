...
// ⚡ Bolt: Memoized recursive FileRow to prevent O(N) re-renders.
// Folders must always re-render to propagate props, but files only re-render if their
// derived `isSelected` state changes, achieving O(1) selection performance.
const FileRow = memo(
  ...
  (prevProps, nextProps) => {
    // Branch nodes (folders) must always re-render (return false) to propagate
    // updated props to children
