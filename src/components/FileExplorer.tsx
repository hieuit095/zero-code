...
const FileRow = memo(function FileRowComponent({ node, depth, selectedId, onSelect }: FileRowProps) {
  ...
  // Folder nodes must always re-render to propagate selectedId down to children
  if (nextProps.node.type === 'folder') return false;

  // For file nodes, only re-render if identity or derived selection state changes
