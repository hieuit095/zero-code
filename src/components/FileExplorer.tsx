...
const FileRow = memo(
  ...
  (prevProps, nextProps) => {
    // ⚡ Bolt: Always re-render folder nodes to correctly propagate props down the tree.
    if (prevProps.node.type === 'folder') return false;

    // ⚡ Bolt: For file nodes, only re-render if its specific selection state changes or fundamental props change.
    ...
    return true; // No relevant changes, skip re-render (O(N) -> O(1))
  }
);
