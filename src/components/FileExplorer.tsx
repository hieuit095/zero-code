...
// ⚡ Bolt: Memoize FileRow to prevent O(N) re-renders
// Folder nodes are not memoized (always return false) to allow selectedId updates to propagate downwards.
// Leaf nodes (files) are strictly memoized based on their isSelected state.
const FileRow = memo(
  ({ node, depth, selectedId, onSelect }: FileRowProps) => {
  ...
  (prevProps, nextProps) => {
    // Branches (folders) must re-render to propagate selectedId down
    if (prevProps.node.type === 'folder') return false;

    // Leaf nodes (files) only re-render if their own selection state changes
    const prevIsSelected =
      prevProps.selectedId === prevProps.node.id || prevProps.selectedId === prevProps.node.name;
    const nextIsSelected =
      nextProps.selectedId === nextProps.node.id || nextProps.selectedId === nextProps.node.name;
...
export function FileExplorer() {
  ...
  // ⚡ Bolt: Stable onSelect callback to prevent FileRow memoization from breaking
