...
function arePropsEqual(prevProps: FileRowProps, nextProps: FileRowProps) {
  // Folders must always re-render to propagate props to their children
  if (prevProps.node.type === 'folder') {
    return false;
  }

  // For files, only re-render if its specific selection state changed
...
export function FileExplorer() {
  ...
  const handleSelectFile = useCallback((id: string, _name: string) => {
