...
const FileRow = memo(function FileRowBase({ node, depth, selectedId, onSelect }: FileRowProps) {
  ...
  // Optimization: Force branch nodes (folders) to always re-render.
  // This is required so they can pass new props (like selectedId) down to their children.
  // Only leaf nodes (files) are memoized based on state changes.
  ...
  // Optimization: Memoize file nodes, only re-rendering if their selection state changes.
  // O(1) rendering cost when switching active tabs.
  return (
