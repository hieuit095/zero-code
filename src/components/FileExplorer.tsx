...
const FileRow = memo(
  ...
  (prevProps, nextProps) => {
    // Folders must always re-render to propagate updated props (like selectedId) down to children
    if (prevProps.node.type === 'folder') {
      return false;
    }

    // Leaf nodes (files) only re-render if their selected state changes or other props change
