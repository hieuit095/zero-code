...
const FileRow = memo(
  ...
  (prevProps, nextProps) => {
    // Folders must always re-render to pass down selectedId to children
    if (prevProps.node.type === 'folder') return false;

    // For leaf nodes (files), only re-render if node identity, depth, or selection state changes
