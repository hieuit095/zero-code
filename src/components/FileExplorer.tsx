...
const FileRow = memo(
  function FileRowImpl({ node, depth, selectedId, onSelect }: FileRowProps) {
  ...
  (prevProps, nextProps) => {
    // ⚡ Bolt: O(1) Re-render Optimization
    // Always re-render folders so they can propagate new props (like selectedId) down to children.
    if (nextProps.node.type === 'folder') return false;

    // For leaf nodes (files), only re-render if its own selection state changes, or if core props change.
...
export function FileExplorer() {
  ...
  // ⚡ Bolt: Memoize file selection to prevent breaking FileRow memoization
