...
const FileRow = memo(
  ...
  (prevProps, nextProps) => {
    // Branch nodes (folders) must always re-render to propagate state down to children.
    if (prevProps.node.type === 'folder' || nextProps.node.type === 'folder') {
      return false;
    }
    // Leaf nodes (files) only re-render if their specific props or selection state changed
