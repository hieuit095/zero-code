...
// ⚡ Bolt: Extracted TaskItem into a memoized component.
// Why: High-frequency task updates and QA retry state changes were causing the entire list
// to re-render in O(N) time. Wrapping this in React.memo() converts the re-render cost
// to O(1) so only the actively changing task will re-render.
...
export function TasksPanel({ tasks, qaRetryState }: TasksPanelProps) {
  ...
  // ⚡ Bolt: Memoized toggle handler with useCallback to ensure stable function identity
  // across renders, preventing unnecessary re-renders of the memoized TaskItem children.
