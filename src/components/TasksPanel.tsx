...
// ⚡ Bolt: Extract inline list items mapped in high-frequency update loops into a memoized component.
// This prevents O(N) re-renders when single tasks update (like QA retries) or when local state (expanded) toggles.
...
export function TasksPanel({ tasks, qaRetryState }: TasksPanelProps) {
  ...
  // ⚡ Bolt: Wrapped toggle in useCallback with empty deps (using functional state updates)
  // so we can pass a stable reference to the memoized TaskItem children.
