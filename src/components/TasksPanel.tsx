...
// ⚡ Bolt: Extracted single task rendering to a memoized component.
// When dealing with high-frequency updates (e.g., active task changing state rapidly),
// extracting list items into React.memo() converts an O(N) re-render bottleneck into
// an O(1) operation where only the actively changing item re-renders.
...
const TaskItem = memo(function TaskItem({ task, isExpanded, isRetrying, toggle }: TaskItemProps) {
  return (
...
export function TasksPanel({ tasks, qaRetryState }: TasksPanelProps) {
  ...
  // ⚡ Bolt: Wrapped toggle function in useCallback to prevent unnecessary re-renders of memoized TaskItems
